...
public class MentorController {
    ...
    public String allMentors(String searchTerm, Model model) {
        List<Mentor> mentors;
    ...
    public String deletementor(@RequestParam("id") int id,HttpSession session) {
        int mentor = mService.deletementor(id);
        if(mentor>0){
            session.setAttribute("deletemessage", "Mentor Deleted Successfully");
        }
        else{
            session.setAttribute("deletemessage", "Unable to delete Mentor");
        }
