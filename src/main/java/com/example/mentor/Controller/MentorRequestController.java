...
public class MentorRequestController {
    ...
    public String approvereq(@RequestParam("mentorid") int mentorid,HttpSession session,HttpServletRequest request) {
        Mentor mentor = mentorRequestService.approveReq(mentorid);
        if(mentor!=null){
            session.setAttribute("approveStatus", "Mentor Approved");
        }
        else{
            session.setAttribute("approveStatus", "Unable to Approve Mentor");
        }
        return "redirect:/allmentorrequest";
    }

    @GetMapping("/rejectrequest")
    public String rejectreq(@RequestParam("mentorid") int mentorid,HttpSession session,HttpServletRequest request) {
        int status = mentorRequestService.disApproveReq(mentorid);
        if(status>0){
            session.setAttribute("rejectStatus", "Mentor Rejected");
        }
        else{
            session.setAttribute("rejectStatus", "Unable to Rejecte Mentor");
        }
        return "redirect:/allmentorrequest" ;
    }
